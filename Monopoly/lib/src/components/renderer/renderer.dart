...
class Renderer {
  ...
  void resizeBoard() {
    ...
    _boardRef.tileWidth =
        _boardRef.tileWidth * 10 ~/ 11; //leave room on left side for buttons
    ...
    for (int i = 0; i < 10; i++) {
      _boardRef.tiles[i]
          .setLocation(_boardRef.x + i * _boardRef.tileWidth, _boardRef.y);
      _boardRef.tiles[i].setSize(_boardRef.tileWidth, _boardRef.tileHeight);
      _boardRef.tiles[i + 10].setLocation(
          10 * _boardRef.tileWidth + _boardRef.x,
          _boardRef.y + i * _boardRef.tileHeight);
      _boardRef.tiles[i + 10]
          .setSize(_boardRef.tileWidth, _boardRef.tileHeight);
      _boardRef.tiles[i + 20].setLocation(
          (10 - i) * _boardRef.tileWidth + _boardRef.x,
          10 * _boardRef.tileHeight + _boardRef.y);
      _boardRef.tiles[i + 20]
          .setSize(_boardRef.tileWidth, _boardRef.tileHeight);
      _boardRef.tiles[i + 30].setLocation(
          _boardRef.x, (10 - i) * _boardRef.tileHeight + _boardRef.y);
      _boardRef.tiles[i + 30]
          .setSize(_boardRef.tileWidth, _boardRef.tileHeight);
    }
  }
  ...
  drawTiles() {
    for (Tile tile in _boardRef.tiles) {
      ...
      _ctxBackground.strokeRect(
          tile.x, tile.y, tile.width, tile.height); //draw tile boarder
      _ctxBackground.fillRect(
          tile.x, tile.y, tile.width, tile.height); //draw tile color
      ...
      if (tile.isMortgaged) {
        ...
        _ctxBackground.fillText(
            'M', tile.x + 9 * tile.width / 10, tile.y + tile.height / 5);
      }
      ...
      _ctxBackground.fillText(
          tile.name, tile.x + tile.width / 2, tile.y + 9 * tile.height / 10);

      //draw buildings
      if (tile.numBuildings == 5) {
        _ctxBackground.fillStyle = 'red';
        _ctxBackground.fillRect(
            tile.x + tile.width / 2 - 10, tile.y + tile.height / 10, 20, 10);
      } else {
        for (int i = 0; i < tile.numBuildings; i++) {
          _ctxBackground.fillStyle = 'green';
          _ctxBackground.fillRect(tile.x + ((i + 1) / 5) * tile.width - 5,
              tile.y + tile.height / 10, 10, 10);
        }
      }
    }
  }
  ...
  void drawPlayer(Player player) {
    ...
    _ctxForeground.arc(
        ...
        (2 / 5) * player.board.tileHeight +
            player.board.tiles[player.currentLocation].y,
        player.size / 2,
        0,
        PI * 2);
    ...
    int infoX = (player.board.x +
            player.board.tileWidth * 1.75 +
            player.number * player.board.tileWidth * 1.25)
        .toInt();
    ...
    _ctxForeground.fillText('\$' + player.money.toStringAsFixed(2), infoX,
        infoY + 37); //display amount of money

    _ctxForeground.font = '10pt sans-serif';
    _ctxForeground.fillText(
